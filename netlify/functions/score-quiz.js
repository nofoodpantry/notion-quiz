  ...
  try {
    ...
    const details = items.map((q) => {
      ...
      if (type.toLowerCase().includes("mcq")) {
        ...
        const accept = (q.correct || "").split(",").map(s => s.trim().toLowerCase()).filter(Boolean);
    ...
    return { statusCode: 200, headers: { "Content-Type": "application/json" }, body: JSON.stringify({ total: items.length, score, details }) };
  } catch (e) {
